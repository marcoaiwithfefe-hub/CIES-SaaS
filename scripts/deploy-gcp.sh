#!/usr/bin/env bash
# GCP Always Free e2-micro — one-shot setup for CIES.
...
# pwuser in mcr.microsoft.com/playwright:jammy runs as UID 1001
sudo chown -R 1001:1001 "$DATA_DIR"

echo "==> Creating swapfile (skipped if already exists)"
if [ ! -f /swapfile ]; then
  ...
  echo "==> 2 GB swapfile created and activated"
else
  echo "==> /swapfile already exists — skipped"
fi
